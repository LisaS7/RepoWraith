...
def _bm25_context(texts: list[str]) -> tuple:
    """Return (document_frequencies, total_docs, average_doc_length) for a corpus of texts."""
    tokenized = [tokenize(t) for t in texts]
    return (
        compute_document_frequencies(tokenized),
        len(tokenized),
        sum(len(t) for t in tokenized) / len(tokenized),
    )
...
def test_bm25_scores_matching_text_higher():
    ...
    doc_freqs, total_docs, avg_len = _bm25_context([matching_text, non_matching_text])
...
def test_bm25_rewards_higher_term_frequency():
    ...
    doc_freqs, total_docs, avg_len = _bm25_context([text_with_one, text_with_three])
...
def test_bm25_score_is_zero_when_query_terms_are_absent():
    corpus_text = "banana lamp velvet octopus"
    doc_freqs, total_docs, avg_len = _bm25_context([corpus_text])
